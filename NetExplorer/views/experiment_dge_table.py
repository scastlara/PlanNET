...
def experiment_dge_table(request):
    ...
    if request.is_ajax():
        ...
        expression = ExpressionRelative.objects.filter(experiment=experiment, dataset=dataset, condition1=condition1, condition2=condition2).order_by('-fold_change')[:100]
        if not expression:
            # In case condition1 and condition2 are reversed in Database
            expression = ExpressionRelative.objects.filter(experiment=experiment, dataset=dataset, condition1=condition2, condition2=condition1).order_by('-fold_change')[:100]
